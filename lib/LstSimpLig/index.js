import { ListaSimpleLigada } from "./lista.js";
import { ListaSimpleLigadaCanvas } from "./listaCanvas.js";
...
export async function insertar_al_inicio() {
    ...
    if (lista == null){
        listaCanvas = new ListaSimpleLigadaCanvas(canvas);
        lista = new ListaSimpleLigada(listaCanvas);
    }

    try{
        let DATO = await bootbox_prompt("INGRESE EL NUEVO VALOR: ");
...
export async function insertar_al_final() {
    ...
    try{
        ...
        let DATO = await bootbox_prompt("INGRSE EL NUEVO VALOR: ");
        if(DATO==null || DATO.trim()=="")
            return;
...
export async function insertar_antes_X() {
    ...
    try{
        ...
        let DATO1 = await bootbox_prompt("INGRESE EL NUEVO VALOR: ");
...
export async function inserta_despues_X() {
    ...
    try{
        ...
        let DATO1 = await bootbox_prompt("INGRESA EL NUEVO NOD: ");
        if(DATO1==null || DATO1.trim()=="")
            return;

        let DATO2 = await bootbox_prompt("INGRSE X: ");
        if(DATO2==null || DATO2.trim()=="")
            return;

        if(lista.buscar(DATO1)==true)
                throw new Error("EL NODO YA ESTA EN LA LISTA");
...
export async function borrar_LstSimpLig() {
    lista = null;
    listaCanvas = null;
    let canvas = window.CANVAS;
